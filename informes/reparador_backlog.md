...
**Resuelto:** 2026-07-26 — añadido `manifest.json` en la raíz (nombre, colores `#6B46E5`/`#F7F7FA` tomados del propio logo de la app, `display: standalone`) e iconos `icon-192.png`/`icon-512.png`/`icon-180.png`/`favicon-32.png` generados vectorialmente (cuadrado redondeado morado + "A" blanca, mismo logo que ya usa la cabecera) sin depender de Pillow (no estaba instalado), con un script auxiliar puramente stdlib que escribe PNG a mano y se borró tras generarlos. Añadido `sw.js` con estrategia stale-while-revalidate para el app-shell (`/`, `support.js`, `banco.js`, `manifest.json`, iconos) — las rutas `/api`, `/auth` y `/asistente` se excluyen explícitamente del cacheo por ser datos dinámicos por usuario. El banco de preguntas ya vive completo en `banco.js` (generado por `scripts/build-banco-js.js`), así que cachear ese fichero ya cubre el "banco de preguntas descargado" del criterio sin lógica adicional por tema. `Tests Oposición.dc.html` actualizado con `<link rel="manifest">`, `<meta name="theme-color">`, icono/apple-touch-icon y el registro del service worker. Verificado manualmente levantando el backend local: `/manifest.json`, `/sw.js`, `/icon-192.png` y `/` responden 200 con el contenido esperado.
...
**Resuelto:** 2026-07-26 — se preguntó al usuario: plan free de Neon, y decidió depender solo de las capacidades nativas de Neon (sin script propio de `pg_dump`/backup externo). Documentada en `CLAUDE.md` (nueva sección "Backups de la base de datos de producción (Neon)") la recuperación nativa de Neon vía historial WAL/point-in-time restore (automática, no requiere activación), los pasos concretos para restaurar creando una branch desde un punto en el tiempo y actualizando `DATABASE_URL` en Render, y el riesgo residual explícito de que la ventana de retención del plan free es corta (recomendación: pasar a plan de pago si esto se vuelve crítico). No se ha tocado código ni configuración de Render — es puramente documentación, tal como pedía el propio criterio de aceptación para el caso de backups ya nativos del proveedor.
